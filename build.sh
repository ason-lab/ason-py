#!/usr/bin/env bash
# build.sh — Build the asun C++ pybind11 extension
...
if [[ ! -f "${PYINC}/Python.h" ]]; then
    echo "[asun-py] Python.h not found at ${PYINC}/Python.h"
    echo "[asun-py] Installing python3-dev ..."
...
echo "[asun-py] Build complete: ${TARGET}"
echo "[asun-py] Run tests: python3 -m pytest tests/ -v"
